...
public class PowerApproximation extends Approximation {
    ...
    public PowerApproximation(double [] x, double [] y) throws Exception {
        graphColor = Color.BLUE;
        this.x = x;
        this.y = y;
        n = x.length;
        if (Arrays.stream(y).min().getAsDouble()<=0 || Arrays.stream(x).min().getAsDouble()<=0)
            throw new Exception("значения у и x не может быть <= 0");
    ...
    private void calculateCoefficients() {
        ...
        double sy = Arrays.stream(y).map(yi -> Math.log(yi)).sum();
        double sxx = Arrays.stream(x)
                .map(x-> Math.log(x)* Math.log(x))
                .sum();
        double sxy = IntStream.range(0, n)
                .mapToDouble(i-> Math.log(x[i])* Math.log(y[i]))
                .sum();
    ...
    public String getStringAnswer(){
        return String.format("\nСтепенная аппроксимирующая функция: %s\n%s\n",
