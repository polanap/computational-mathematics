...
public class ExponentialApproximation extends Approximation {
    ...
    public ExponentialApproximation(double [] x, double [] y) throws Exception {
    ...
    public String toString() {
        ...
        return String.format("%s * e ^ (x*%s)", decimalFormat.format(a), decimalFormat.format(b));
    }

    @Override
    public String getStringAnswer(){
        return String.format("\nЭкспоненциальная аппроксимирующая функция: %s\n%s\n",
    ...
    public void plotGraph(LineChart<Number, Number> lineChart) {
        ...
        double right = Arrays.stream(x).max().getAsDouble() + 1;
